...
class App extends React.Component {
  ...
  handleChange = (event) => {
    ...
    handleSubmit = (event) => {
      ...
      axios.post('/api/quiz', this.state).then((response) => {
        console.log(response.data)
        this.setState({
          quizq: [response.data],
          question: response.data.question,
          answer: response.data.answer,
          answer_char: response.data.answer_char.toUpperCase(),
    ...
    updateQuestion = (event) => {
      ...
      axios.put('/api/quiz/' + id, this.state).then((response) => {
        this.getQuestion()
      })
    }

    deleteQuestion = (event) => {
      axios.delete('/api/quiz/' + event.target.value).then((response) => {
        this.getQuestion()
      })
    }
    ...
    getQuestion = () => {
      ...
      .then(
        (response) => this.setState({
          ...
          answer_char: response.data[0].answer_char.toUpperCase(),
